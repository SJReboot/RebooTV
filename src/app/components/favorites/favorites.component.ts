    ...
    <div class="h-full flex flex-col">
      ...
      <div class="p-6 pb-4 flex-shrink-0">
        <div class="flex items-center justify-between gap-4 mb-4">
          <h2 class="text-3xl font-bold text-gray-200">{{ isMassEditing() ? 'Edit Favorites' : (iptvService.showHiddenChannels() ? 'Hidden Favorites' : 'Favorites') }}</h2>
      ...
      <div class="flex-1 overflow-y-auto px-6" (scroll)="onScroll($event)">
        ...
        @if (!isLoading() && channels().items.length === 0 && !error()) {
          <p class="text-gray-400 col-span-full text-center mt-8">{{ iptvService.showHiddenChannels() ? 'No hidden favorites found.' : "You haven't added any channels to your favorites yet." }}</p>
        }
      </div>
