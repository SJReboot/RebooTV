    ...
    <div class="h-full flex flex-col">
      ...
      <div class="p-6 pb-4 flex-shrink-0">
        <div class="flex items-center justify-between gap-4 mb-4">
          <h2 class="text-3xl font-bold text-gray-200">{{ isMassEditing() ? 'Edit History' : (iptvService.showHiddenChannels() ? 'Hidden History' : 'Recently Watched') }}</h2>
      ...
      <div class="flex-1 overflow-y-auto px-6" (scroll)="onScroll($event)">
        ...
        @if (error()) {
            <div class="col-span-full text-center p-8 text-red-400">
                <p>Error loading recently watched channels: {{ error() }}</p>
        ...
        @if (!isLoading() && channels().items.length === 0 && !error()) {
          <p class="text-gray-400 col-span-full text-center mt-8">{{ iptvService.showHiddenChannels() ? 'No hidden items in your history.' : "You haven't watched any channels yet." }}</p>
        }
      </div>
  ...
  fetchData(loadMore: boolean) {
    ...
    const options = {
        ...
        filter: {
          ...
          categoryId: this.iptvService.selectedCategoryId()
        },
