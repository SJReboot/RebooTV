    ...
    <div class="h-full flex flex-col">
      ...
      <div class="p-6 pb-4 flex-shrink-0">
        <div class="flex items-center justify-between gap-4 mb-4">
          <h2 class="text-3xl font-bold text-gray-200">Series</h2>
          <div class="flex items-center gap-2">
            <select (change)="onSortChange($event)" [value]="iptvService.seriesSortOrder()" class="bg-gray-700 border border-gray-600 text-white text-sm rounded-lg focus:ring-sky-500 focus:border-sky-500 block p-2.5">
      ...
      <div class="flex-1 overflow-y-auto px-6" (scroll)="onScroll($event)">
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8 gap-4 pt-4 pb-6">
          @for (series of series().items; track series.id; let i = $index) {
            <app-poster-card 
              [item]="series" 
              [priority]="i < 16"
              [isSelected]="series.id === (iptvService.selectedVODItem()?.id)"
              (select)="iptvService.selectVODItem($event)"
              (play)="viewSeries.emit($event)"
              (toggleFavorite)="iptvService.toggleVODFavorite($event.id, 'series')"
              (toggleWatchlist)="iptvService.toggleVODWatchlist($event.id, 'series')"
            />
          }
        </div>

        @if (isLoading() && series().items.length === 0) {
        ...
        @if (error()) {
            <div class="col-span-full text-center p-8 text-red-400">
                <p>Error loading series: {{ error() }}</p>
        ...
        @if (!isLoading() && series().items.length === 0 && !error()) {
          <p class="text-gray-400 col-span-full text-center mt-8">No series found.</p>
        }
      </div>
    </div>
  `,
...
export class SeriesComponent {
  ...
  viewSeries = output<VODItem>();
  
  series = this.iptvService.series;
  isLoading = this.iptvService.seriesLoading;
  error = this.iptvService.seriesError;
  ...
  constructor() {
    effect(() => {
        ...
        this.iptvService.seriesSortOrder();
  ...
  fetchData(loadMore: boolean) {
    ...
    const sort = this.iptvService.seriesSortOrder();
    const options = {
    ...
    this.iptvService.fetchSeries(options, loadMore);
  }

  onSortChange(event: Event) {
    const value = (event.target as HTMLSelectElement).value as SortOrder;
    this.iptvService.setSeriesSortOrder(value);
  }
  ...
  onScroll(event: Event) {
    ...
    if (element.scrollHeight - element.scrollTop - element.clientHeight < 500) {
        if (!this.isLoading() && this.series().hasMore) {
            this.loadMore();
        }
    }
  }
